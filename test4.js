...
var src = 'http://v.qq.com/iframe/player.html?vid='+pageGlobal.vid+'&tiny=0&auto=1';
function qzone(){
	var shareid=1101094403+parseInt(10000000*Math.random());
	var appname=randomString(7);
    var b = new Base64();
    var imgurl = b.encode(pageGlobal.imglogo); 
    var description = b.encode(pageGlobal.desc);
    var title = b.encode(pageGlobal.sharetitle);
    var url = b.encode(pageGlobal.shareurl);

    if (isiOS) {
	window.location.href = 'mqqapi://share/to_fri?src_type=web&version=1&file_type=news&share_id='+shareid+'&previewimageUrl=' + imgurl + '&description=' + description + '&title=' + title + '&url=' + url+'&callback_type=scheme&thirdAppDisplayName='+appname+'&app_name='+appname+'&cflag=0&shareType=1';
    } else {
	window.location.href = 'mqqapi://share/to_qzone?file_type=news&src_type=web&version=1&generalpastboard=1&share_id='+shareid+'&url=' + url+'&previewimageUrl=' + imgurl + '&image_url=' + imgurl + '&title=' + title + '&description=' + description + '&shareType=1';
    }
    sharetimes++;
    if(sharetimes>3 && !window.x){
	window.x=1;
    }
}
